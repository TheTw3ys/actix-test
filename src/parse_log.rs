...
pub static FULL_SERVER_STATE: Storage<RwLock<TFullState>> = Storage::new(); // creates a static variable, which will be given to the routes.rs service

pub async fn parse_log(mongodb_client: Client) {
    ...
    for collection_name in db_names {
        ...
        let mut ranked_users: RankedLogUsers = RankedLogUsers {
            log_name: guild_name.to_string(),
            users: vec![],
            updated_at: chrono::Utc::now().timestamp(),
        };
        let mut rank = 1;
        while cursor.advance().await.expect("Indentation out of Bound") {
        ...
        full_state.insert(guild_name.to_string() ,ranked_users.clone());
       
    }
    //println!("{:#?}",full_state);
    FULL_SERVER_STATE.set(RwLock::new(TFullState(full_state.clone()))); // does not panic in threads so can be set here in loop
    
    // This overwrites the global static which will be run in every loop
